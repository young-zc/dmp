...
import top.newforesee.bean.ad.{AdBlackList, AdClickTrend, AdProvinceTop3, AdStat, AdUserClickCount}
import top.newforesee.dao.ad.{IADUserClickCountDao, IAdBlackListDao, IAdClickTrendDao, IAdProvinceTop3Dao, IAdStatDao}
...
object AdFlowRealTimeCalJob {
  ...
  def calSlidewindow(whiteList: DStream[String]): Unit = {
    ...
    whiteList.map((item: String) => {
      ...
      (time + "#" + adId, 1L)
    }).reduceByKeyAndWindow((v1: Long, v2: Long) => {
      v1 + v2
  ...
  def calPerDayProviceHotADTop3(dsClickCnt: DStream[(String, Long)]): Unit = {
    ...
    dsClickCnt.map((perEle: (String, Long)) => {
      ...
      ssc.sql("select *,row_number() over(partition by province order by click_count desc) rank " +
        "from temp_ad_province having rank<=3")
